...
프로토타입으로 서비스 내 가장 무거운 쿼리를 전환한 결과, 속도는 2~3배 향상되고 메모리 사용량은 82% 감소했다. Roslyn Source Generator를 활용해 빌드타임에 매핑 코드를 생성함으로써 런타임 리플렉션을 완전히 제거하고, EF Core가 구조적으로 지원하지 못하는 Native AOT 환경과 WDAC(Windows Defender Application Control) 환경에서도 동작하는 SQL 매퍼를 만들었다.
...
- 런타임 리플렉션 제로, Native AOT 완전 호환 (.NET 8+) — 현존하는 유일한 MyBatis 방식의 AOT 호환 SQL 매퍼
...
| Native AOT 환경 (.NET 8+) | Source Generator가 빌드타임에 모든 매핑 코드를 생성한다. 런타임 리플렉션이 없으므로 AOT 트리밍에서 안전하다 |
...
| `NuVatis.SqlServer` | SQL Server/MSSQL Provider (Microsoft.Data.SqlClient) |
...
| `NuVatis.QueryBuilder` | jOOQ 스타일 타입 안전 SQL DSL (PostgreSQL · MySQL · SQL Server · Oracle 방언) |
| `NuVatis.QueryBuilder.Tools` | dotnet global tool — DB 스키마 스캐너 + QueryBuilder 코드 생성기 |
...
BenchmarkDotNet으로 NuVatis, Dapper, EF Core를 60개 시나리오에서 비교 측정했다. 아래는 대표 시나리오의 요약이다.
...
전체 60개 시나리오의 상세 결과는 [벤치마크 대시보드](https://jinho-von-choi.github.io/nuvatis-sample)에서 확인할 수 있다.
...
| `<selectKey>` | INSERT 전/후 키 생성 SQL 실행 — 생성된 PK를 파라미터 객체에 자동 주입 |

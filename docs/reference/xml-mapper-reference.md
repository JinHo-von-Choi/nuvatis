...
```xml
<select id="GetById" resultMap="UserResult" timeout="30">
...
| `timeout` | 아니오 | 팩토리 기본값 | Statement별 실행 타임아웃 (초). 기본값은 `NuVatisOptions.DefaultCommandTimeout` |

> 캐시 적용 여부는 per-statement 속성이 아니라 namespace 레벨의 `<cache>` 요소로 제어한다.
...
| `timeout` | 아니오 | 팩토리 기본값 | 실행 타임아웃 (초) |

**Auto-Increment Key 반환 — selectKey**

INSERT 후 생성된 PK를 파라미터 객체에 자동 주입한다. `<selectKey>` 요소를 사용한다:

```xml
<insert id="Insert">
  <selectKey keyProperty="Id" order="After">
    SELECT lastval()   <!-- PostgreSQL: 마지막 생성 시퀀스 값 -->
  </selectKey>
...
// user.Id에 DB가 생성한 PK 값이 주입되어 있다
```
...
| 속성 | 필수 | 설명 |
|------|------|------|
| `keyProperty` | 예 | PK 값을 설정할 파라미터 객체의 프로퍼티명 |
| `order` | 아니오 | `Before` 또는 `After` (기본: `After`). INSERT 전/후 실행 시점 |
| `resultType` | 아니오 | 반환 C# 타입명. 생략 시 반환값 타입을 그대로 사용 |

`order="Before"` 는 시퀀스 기반 DB (Oracle의 `sequence.NEXTVAL` 등)에서 INSERT 전에 키를 생성할 때 사용한다.

**PostgreSQL RETURNING 방식 (대안)**

`selectKey` 없이 `RETURNING` 절과 별도 `<select>` 를 조합하는 방식도 사용 가능하다:
